...
public class WXPayUtil {
	...
	public static SortedMap<String, String> getPackage(WxPayDto wxPayDto) {
		...
		contentMap.put("appid", PayConstants.WX_APPID);
		contentMap.put("body", wxPayDto.getBody());
		contentMap.put("input_charset", "UTF-8");
		contentMap.put("mch_id", PayConstants.WXMCH_ID);
		...
		contentMap.put("notify_url", PayConstants.WXPAY_NOTIFY_URL);

		contentMap.put("trade_type", "APP");
		//contentMap.put("openid", openId);

		RequestHandler reqHandler = new RequestHandler(null, null);
		reqHandler.init(PayConstants.WX_APPID, PayConstants.WXAPPSECRET, PayConstants.WX_PARTNERKEY);

		String sign = reqHandler.createSign(contentMap);
		contentMap.put("sign", sign);
		String xml = WXPayUtil.paramsToxmlStr(contentMap);
		
		String prepay_id = new GetWxOrderno().getPayNo(PayConstants.PREPAY_ID_URL, xml);
		...
		contentMap.put("partnerid", PayConstants.WXMCH_ID); 
		contentMap.put("prepayid", prepay_id); 
		contentMap.put("timestamp", timestamp);  
		
		
		
		payMap.put("appId", PayConstants.WX_APPID);  
		payMap.put("partnerId", PayConstants.WXMCH_ID); 
	...
	public static String createSign(TreeMap<String, String> treeMap,String partnerKey) {
		...
		String sign = MD5Util
				.MD5Encode(stringSignTemp, PayConstants.INPUT_CHARSET)
				.toUpperCase();
		return sign;
	}
	/**
